...
[tck-testable]#The `type-argument-index` can be omitted, if the container element type has exactly one type argument.#
...
If an invalid container type element configuration is detected, a `ValidationException` is raised.
...
[role="api"]
.`ValueExtractorDefinitionException` exception
====
[source,java]
----
package javax.validation.valueextraction;
...
[role="api"]
.`ValueExtractorDeclarationException` exception
====
[source,java]
----
package javax.validation.valueextraction;
