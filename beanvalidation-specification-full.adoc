...
** Support for custom container types by plugging in additional value extractors (see <<constraintdeclarationvalidationprocess-valueextractordefinitions>>)
...
Container element constraints must not be put to wildcard type arguments (with or without bounds), i.e. the following usage is disallowed:

.Disallowed usage of a container element constraint on a wildcard type argument
====
[source, JAVA]
----
...
Only wildcard type arguments may be annotated with `@ExtractedValue`.
...
** [tck-testable]#if the traversed object is of a container type (e.g. a `List`, `Map` or `Optional`), the following `Node` object added returns the type of the traversed container via `getContainerClass()` and the index of the type argument to which the constraint applies via `getTypeArgumentIndex()#
...
** a [classname]`ContainerElementNode` object is added to [classname]`Path` whose `name` is the node name produced by the corresponding value extractor (see <<constraintdeclarationvalidationprocess-valueextractordefinitions>>).
...
* one or more value extractor instance(s)
...
`getConstraintsForContainerElementType()` returns a descriptor representing the constraints of the specified type argument.
`getConstrainedContainerElementTypes()` returns descriptors for all those type arguments that are hosting at least one constraint or are marked for cascaded validation.

The `ContainerElementTypeDescriptor` interface describes container element constraints applied to one type argument of a container type.

This interface is returned by `ContainerDescriptor.getConstraintsForContainerElementType()` and `ContainerDescriptorgetConstrainedContainerElementTypes()`.
...
[source, JAVA, indent=0]
----
Unresolved directive in sources/constraint-metadata.asciidoc - include::../target/validation-api/javax/validation/metadata/ValidateUnwrappedValue.java[lines=7..8;11..-1]
----
...
<xs:schema attributeFormDefault="unqualified"
    ...
    <xs:complexType name="container-element-typeType">
        <xs:sequence>
            ...
            <xs:element type="map:constraintType"
                        ...
                        maxOccurs="unbounded"
                        minOccurs="0"/>
            <xs:element type="map:container-element-typeType"
...
[tck-not-testable]#When a constraint declaration is illegal, [classname]`ConstraintDeclarationException` is raised.# This includes among other things incorrect group conversion rules (definition or positioning) and illegal method constraint declarations (e.g. inheritance rules, cross-parameter constraint used in an illegal situation, improper use of [methodname]`validationAppliesTo`).
...
[tck-testable]#If a custom [classname]`ConstraintValidatorFactory`, [classname]`MessageInterpolator`, [classname]`ParameterNameProvider`, [classname]`ClockProvider` or [classname]`TraversableResolver` class is defined in the XML deployment descriptor (see <<validationapi-bootstrapping-xmlconfiguration>>), the [classname]`ValidatorFactory` must be configured with CDI managed beans representing the requested classes. Services like dependency injection, interception and decoration must thus be made available to these instances by the container.#
