...
[classname]`Path` is an iterable of [classname]`Node` objects.
In addition to iteration, [classname]`Path` offers the following methods
for direct access to its nodes:

* [tck-testable]#[methodname]`length()` returns the number of nodes in the path.#
* [tck-testable]#[methodname]`getNode(int index)` returns the node at the given index.
Throws [classname]`IndexOutOfBoundsException` if the index is negative
or greater than or equal to [methodname]`length()`.#
* [tck-testable]#[methodname]`getRootNode()` returns the first node in the path.
Equivalent to [methodname]`getNode(0)`.#
* [tck-testable]#[methodname]`getLeafNode()` returns the last node in the path.#

[NOTE]
====
The default implementations of these methods iterate through nodes
to produce the result. Implementations may provide more optimal
versions, but are not required to do so.
====

.Accessing path nodes: iterator vs direct access
====
...
// Using iterator-based access:
Iterator<Node> nodeIter = path.iterator();
Node first = nodeIter.next();
Node second = nodeIter.next();

// Using direct access:
Node first = path.getRootNode();
Node second = path.getNode(1);
Node last = path.getLeafNode();
int pathLength = path.length();
----

====

[classname]`Node` offers the following methods:
...
<validation-config
        ...
        xsi:schemaLocation="https://jakarta.ee/xml/ns/validation/configuration
            https://jakarta.ee/xml/ns/validation/configuration/validation-configuration-4.0.xsd"
        version="4.0">
