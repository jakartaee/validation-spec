...
The default package for the {spec-name-bv} APIs is [classname]`javax.validation`.

[[validationapi-validatorapi]]

=== Validator API

The main {spec-name-bv} API is the [classname]`javax.validation.Validator` interface.
...
[tck-testable]#Every {spec-name-bv} provider must provide a [classname]`ValidationProvider` implementation containing a public no-arg constructor# and add the corresponding [filename]`META-INF/services/javax.validation.spi.ValidationProvider` file descriptor in one of its jars.
