...
The former returns the class of the container in case the constrained element is contained in a container type such as `Optional`, `List` or an array.
The latter returns the index of the affected type argument in case the constrained element is contained in a generic type such as `Optional`, `List` or `Map`.
--
...
case CONTAINER_ELEMENT:
    ContainerElementNode containerElementNode = node.as(ContainerEleemntNode.class);
...
** [tck-testable]#if the traversed object is a container type (e.g. a `List`, `Map`, `Optional` or array), the following `Node` object added returns the type of the traversed container via `getContainerType()`#
** [tck-testable]#if the traversed object is generic container type (e.g. a `List` or `Map`, `Optional`), the following `Node` object added returns the index of the type argument to which the constraint applies via `getTypeArgumentIndex()`#
