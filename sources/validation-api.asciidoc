...
** [tck-testable]#if the traversed object is of a container type (e.g. a `List`, `Map`, `Optional` or array), the following `Node` object added returns the type of the traversed container via `getContainerClass()`#
** [tck-testable]#if the traversed object is of a generic container type (e.g. a `List` or `Map`, `Optional`), the following `Node` object added returns the index of the type argument to which the constraint applies via `getTypeArgumentIndex()`#
