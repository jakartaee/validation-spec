...
For applying constraints to the elements of generic container types or marking the same for cascaded validation, the `containerElementType` element is used.

`containerElementType` can be used within the `field`, `getter`, `parameter` and `return-value` elements.
The `typeArgumentIndex` is used to specify the index of the configured type argument.
The `ignore-annotations` settings effectively applying for the encapsulating element (`field`, `getter` etc.) are applied for `containerElementType`, too.
The `containerElementType` element can be nested for configuring nested generic containers such as `List<List<String>>`.
Constraints are applied by adding `constraint` elements to `containerElementType`.
