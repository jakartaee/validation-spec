...
** If there are several extractors which are equally specific, an `ConstraintDeclarationException` is raised.
...
** If there are multiple such extractors, an `ConstraintDeclarationException` is raised.
