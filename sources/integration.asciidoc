...
There are several integrations points between Bean Validation and CDI. If a Bean Validation provider integrates with CDI, it must follow the rules laid out in this section. In a {spec-name-eeplatform} container, a [tck-testable]#Bean Validation provider must integrate with CDI.#
