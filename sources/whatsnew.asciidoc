...
* The namespace for Bean Validation XML descriptors has been changed to `https://xmlns.jakarta.ee/xml/ns/validation/configuration` for `META-INF/validation.xml` and `https://xmlns.jakarta.ee/xml/ns/validation/mapping` for constraint mapping files (see <<validationapi-bootstrapping-xmlconfiguration>>)
