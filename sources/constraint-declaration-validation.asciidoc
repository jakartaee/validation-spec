...
provided a value extractor implementation (see <<constraintdeclarationvalidationprocess-valueextractordefinitions>>) for that container type is present.
Amongst others, there are built-in value extractors for the generic collection types listed above (see <<constraintdeclarationvalidationprocess-valueextractordefinitions-builtinvalueextractors>>).
...
[source, JAVA]
----
include::{validation-api-source-dir}javax/validation/GroupSequence.java[lines=18..-1]
----
...
[source, JAVA, indent=0]
----
include::{validation-api-source-dir}javax/validation/groups/ConvertGroup.java[lines=7..8;25..-1]
----
...
Constraints can be applied to the elements of container types, e.g. `List`, `Map`, `Optional` or arrays.
This is done by annotating type arguments in the case of generic container types and by annotating the component type in the case of arrays.
Container element constraints can be applied to containers that are
...
public Person(String @Email @NotNull [] emails) {
    [...]
}
----
====

When a field, property, executable parameter or method return value is validated,
any container element constraints of that element will be validated alongside any other other constraints hosted by that element.

When a container element constraint is detected, the validation engine invokes a value extractor (see <<constraintdeclarationvalidationprocess-valueextractordefinitions>>)
...
or multiple values in the case of collection types or arrays.
...
An example are JavaFX's non-generic property types such as `StringProperty` or `IntegerProperty`:
...
Here the `@Size` and `@Positive` constraints don't apply to the annotated `StringProperty` and `IntegerProperty` themselves, but rather to the wrapped string and integer values, respectively.

For this to work, an unambiguously resolvable value extractor must be defined which carries the `@UnwrapByDefault` annotation (see <<constraintdeclarationvalidationprocess-valueextractordefinitions-unwrapbydefault>>).

If needed, the target (container or container element) of a constraint declared on a container can be explictly specified via the `Unwrap` and `Skip` payload definitions:
...
[[constraintdeclarationvalidationprocess-valueextractordefinitions]]
=== Value extractor definitions
...
When passing a non-null node name to any of the receiver methods and the extractor is called in the course of container element validation (and not during cascaded validation),
a node of kind `CONTAINER_ELEMENT` will be appended to the property path, with `Node#getName()` returning the specified name.
If null is passed as node name, no node will be appended to the property path in this case.
The resulting property path will then be the same as if the constraint had been given on container instead of a container element.
...
[[constraintdeclarationvalidationprocess-valueextractordefinitions-extractedvalue]]
==== @ExtractedValue
...
For generic container types `@ExtractedValue` is to be specified on a type argument of the container type as used in the extractor definition.
For array container types `@ExtractedValue` is to be specified on the array component type as used in the extractor definition.
...
[[constraintdeclarationvalidationprocess-valueextractordefinitions-unwrapbydefault]]
==== @UnwrapByDefault

Value extractor definitions can be marked with the `@UnwrapByDefault` annotation.
This causes constraints are automatically applied to the wrapped value if a constraint is found for an element handled by that extractor (see <<constraintdeclarationvalidationprocess-containerelementconstraints-implicitunwrapping>>):
...
[[constraintdeclarationvalidationprocess-valueextractordefinitions-builtinvalueextractors]]
==== Built-in value extractors

Compatible implementations provide value extractors for the following types out of the box:

* Arrays of objects and arrays of all primitive Java data types (`boolean[]`, `int[]` etc.); `indexedValue()` must be invoked for each array element, passing the literal `<iterable element>` as node name
...
* `java.util.List`; `indexedValue()` must be invoked for each contained element, passing the string literal `<iterable element>` as node name
...
* `javafx.beans.observable.ObservableValue`; `value()` must be invoked for the observable's value, passing `null` as node name; the extractor must be marked with `@UnwrapByDefault`
* `javafx.beans.property.ReadOnlyListProperty` and `javafx.beans.property.ListProperty`; `indexedValue()` must be invoked for each contained element, passing the string literal `<iterable element>` as node name
...
A value extractor for the elements of an array of `int`:
...
* Provided by the validation engine itself (see <<constraintdeclarationvalidationprocess-valueextractordefinitions-builtinvalueextractors>>)
...
A value extractor for a given type and type parameter specified at a higher level overrides any other extractors for the same type and type parameter given at lower levels.
...
[[constraintdeclarationvalidationprocess-validationroutine-algorithm-constraints]]
...
* A `ValueExtractor` implementation is said to be _container-element-compliant_ with `C`,
** if `C` is a generic container type and the value extractor implementation handles a type parameter that maps to the constrained type argument
** if `C` is an array container type and the value extractor implementation is type-compliant with `C`
* If no `ValueExtractor` type-compliant and container-element-compliant with `C` is found amongst the available value extractors, a `UnexpectedTypeException` is raised.
...
* If more than one maximally specific and container-element-compliant `ValueExtractor` is found, an `UnexpectedTypeException` is raised.

[NOTE]
.Implementation note
====
Extractor retrieval for container element constraints is based on the declared type of constrained elements,
hence it is recommended that implemenations perform the resolution once and then cache the value extractor for a given constraint.
====

[[constraintdeclarationvalidationprocess-validationroutine-algorithm-cascaded]]
...
* If no `ValueExtractor` type-compliant with `C'` and container-element-compliant with `C` is found amongst the available value extractors, a `UnexpectedTypeException` is raised.
* A `ValueExtractor<U>` type-compliant with `C'` is considered _strictly more specific_ than a `ValueExtractor<V>` compliant with `C'` if `U` is a strict subtype of `V`.
`U` is a strict subtype of `V` if `U` is a subtype of `V` and `U` != `V`.
* A `ValueExtractor<U>` type-compliant with `C'` is considered maximally specific if no other `ValueExtractor<V>` type-compliant with `C'` is strictly more specific than `ValueExtractor<U>`.
* If more than one maximally specific and container-element-compliant `ValueExtractor` is found, an `UnexpectedTypeException` is raised.
...
(this is to be consistent with the semantics of property paths for cascaded validation defined in Bean Validation 1.x),
hence it is not possible to perform the extractor resolution only once and cache the result.
It is recommend though that implementations perform a pre-analysis of applicable extractors based on the declared type,
requiring to process only a sub-set of all available extractors at runtime.
====
...
* If the constraint carries the `Unwrapping.Unwrap` payload and there is exactly one most-specific type-compliant value extractor, this extractor is applied;
if there is no type-compliant extractor or multiple most-specific type-compliant extractors exist, a `ConstraintDeclarationException` is raised
* If the constraint carries neither the `Unwrapping.Unwrap` nor the `Unwrapping.Skip` payload:
** If there is exactly one most-specific type-compliant value extractor marked with `@UnwrapByDefault`, this extractor is applied
...
public class IntArrayValueExtractor implements ValueExtractor<int @ExtractedValue []> {
    [...]
}

public class LongArrayValueExtractor implements ValueExtractor<long @ExtractedValue []> {
    [...]
}
...
|`int Min(1) [] positiveInts`|`IntArrayExtractor`
...
|`Optional<@Email String> getEmail() {...}`|`UnexpectedTypeException`; no compliant extractor exists
|`Table<@Min(1) String, String, String> table`|`UnexpectedTypeException`; `TableValueExtractor` is type-compliant but not container-element-compliant
|`MyList<@Email String> emails`|`UnexpectedTypeException`; `ListValueExtractor` and `ConcurrentListValueExtractor` are equally specific
|`SingleTypeMap<@NotEmpty String> map`|`UnexpectedTypeException`; `MapKeyExtractor` and `MapValueExtractor` are equally specific and both container-element-compliant
|`@NotEmpty(payload=Unwrapping.Unwrap.class) StringMap`|`UnexpectedTypeException`; more than one most-specific extractor is found (`MapKeyExtractor` and `MapValueExtractor`)
|===============
...
[source, JAVA, indent=0]
----
include::{spec-examples-source-dir}org/beanvalidation/specexamples/constraintdeclarationvalidation/resolution/NotEmpty.java[tags=include]
----
