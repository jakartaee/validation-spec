...
* [tck-testable]#the Bean Validation provider treats regular annotations (annotations not annotated by [classname]`@Constraint`) whose `value` element has a return type of an array of constraint annotations in a special way. Each element in the `value` array are processed by the Bean Validation implementation as regular constraint annotations.# This means that each constraint specified in the `value` element is applied to the target. The annotation must have retention `RUNTIME` and can be applied on a type, field, property, executable parameter, executable return value, executable cross-parameter or another annotation. It is recommended to use the same set of targets as the initial constraint.
* [tck-testable]#the Bean Validation provider supports constraints annotated with the [classname]`@Repeatable` annotation. Each repeated constraint is applied to the target.#

Note to constraint designers:

* each constraint annotation should be coupled with its corresponding multi-valued annotation. The specification recommends, though does not mandate, the definition of an inner annotation named [classname]`List`.
* each constraint annotation should be annotated with the [classname]`@Repeatable` annotation. The specification recommends, though does not mandate, the use of the aforementioned [classname]`List` class as the container of the [classname]`@Repeatable` annotation.
...
.Multi-valued constraint declaration by repeating the annotation
====
...
.Multi-valued constraint declaration using a multi-constraint annotation
====
...
In these examples, both constraints apply to the [methodname]`zipcode` field but with different groups and with different error messages.
