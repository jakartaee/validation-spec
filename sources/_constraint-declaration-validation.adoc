...
but not both.
[tck-not-testable]#This specification doesn't define the behaviour for having the `@Valid` annotation on the container _and_ on the type argument(s) of the container.
A specific implementation may, for example, execute the validation twice.#
...
|[code]`@Size SerializableCollection getAddresses() { [...] }`|[classname]`UnexpectedTypeException`:
              ...
              other.
|[code]`@Size String getName() { [...] }`|[classname]`SizeValidatorForSerializable`:
              [classname]`String` is a subtype of [classname]`Serializable`.
|===============
